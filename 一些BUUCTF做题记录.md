...
尝试上传了图片马，发现是可以上传成功的，猜测它并不会检查图片中是否包含一句话木马

查看源代码，发现是前端验证

![72723366926](assets/1727233669268.png)

删去审查的函数，但是仍旧无法上传，感觉这里有些不大一样跟别人的博文
...
利用bp抓包，上传shell.php木马，将木马的后缀名修改为png，并且使注入的一句话木马被解析成png文件(忽略截图中的后缀是jpg)
...
auto_prepend_file=shell.png       //需要注意上穿的文件名字要和设定的名字一样，这里可以设定多个
```
...
![72821890177](assets/1728218901776.png)

拿到Flag

![72821894078](assets/1728218940781.png)
...
常见的备份文件后缀名有 .git .[svn](https://so.csdn.net/so/search?q=svn&spm=1001.2101.3001.7020) .swp .~ .bak .bash_history
于是用dirsearch扫描目录

```
python dirsearch.py -e php,bak -u http://efd5c039-d585-4243-affb-0f5263ac55e9.node5.buuoj.cn:81/
```

![72753855716](assets/1727538557161.png)看到bak的后缀(或者一个一个试一试)

![72753220850](assets/1727532208506.png)

得到一段源码：

![72753223195](assets/1727532231950.png)

代码审计：

​	intval函数获取变量的整数值，因此str=123

> 构造Payload：http://efd5c039-d585-4243-affb-0f5263ac55e9.node5.buuoj.cn:81/?key=123

成功得到flag

![72753230585](assets/1727532305853.png)

这道题难在如何找到源码文件，找到了就是一道很简单的题目了
